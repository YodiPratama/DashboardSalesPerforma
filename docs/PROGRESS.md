...
| Tanggal    | Perubahan |
|------------|-----------|
| 2026-07-25 | **Customer status: gabung Lost+Dormant, tambah New (NOO)**: status `Dormant` dihapus, digabung ke `Lost` (keduanya sama-sama "tidak order bulan ini", cuma beda lama tidaknya). Ditambah status baru `New` (New Open Outlet / NOO) — customer yang order bulan ini tapi belum pernah beli sama sekali sebelumnya (pakai window "ever" yang sama dengan `newOutlet` di KPI Overview). Disentuh: `analytics.js` (`calcCustomerTable`), `section-customer.js` (summary card + donut chart), `table.js` (badge, filter pill, kolom status tabel), `modal.js` (badge detail customer). Status Summary di Customer Analytics sekarang: Active, New (NOO), Reactivated, At Risk, Lost. |
